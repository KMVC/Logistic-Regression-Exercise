...
#examine the structure of ever worked
str(NH11$everwrk)
summary(NH11$everwrk)

#examine the levels of ever work
levels(NH11$everwrk)
#remove Not ascertained as it has 0 responses, and collapse all missing values to NA
NH11$everwrk =  factor(NH11$everwrk, levels=c("2 No", "1 Yes"))
summary(NH11$everwrk)

#take a look at r_maritl
str(NH11$r_maritl)
summary(NH11$r_maritl)
#Need to remove 0-14 years and Married- spouse in household unknown because they got 0 responses
...
dideverwork = glm(everwrk ~ age_p + r_maritl, data = NH11, family = "binomial")
coef(summary(dideverwork))

#make the regression easier to interpret
dideverwork.tab <- coef(summary(dideverwork))
dideverwork.tab[, "Estimate"] <- exp(coef(dideverwork))
dideverwork.tab
...
cbind(predDatever, predict(dideverwork, type = "response",
                       se.fit = TRUE, interval="confidence",
                       newdata = predDatever))
library(effects)
